...
void copy_matrix(double* mat1, double* mat2, size_t rows1, size_t cols1, size_t cols2);
...
{ 
 ...
 n_loc_min = n/np;
 double *A_loc, *B_loc, *C_loc, *B_temp, *C_temp;
 ...
 C_temp = (double *) malloc(sizeof(double)*n_loc*n_loc_max); 
 memset(C_temp, 0, sizeof(double)*n_loc*n_loc_max);
 ...
 #ifdef __CUDA
  double cpy_time = 0;
  double gpu_init_start = MPI_Wtime();
  cuda_initialize(A_loc, n_loc, n_loc_max, n, rank, &cpy_time);
  double gpu_init_end = MPI_Wtime();
  double init_time  = gpu_init_end - gpu_init_start - cpy_time;
  double t_cpy_Aloc = cpy_time;
  double all_init_time = 0, all_cpy_time =0, start_cpy_time, end_cpy_time;
 #endif
...
#ifdef __CUDA
  if(i < res) {
    cuda_vector_mul(B_temp, &C_temp, n_loc, n_loc_max, n, &cpy_time, &calc_time);
    start_cpy_time = MPI_Wtime();
    copy_matrix(C_temp, C_loc+(i*n_loc_max), n_loc, n_loc_max, n);
    end_cpy_time = MPI_Wtime(); 
  }
  else{
    cuda_vector_mul(B_temp, &C_temp, n_loc, n_loc_min, n, &cpy_time, &calc_time);
    start_cpy_time = MPI_Wtime();
    copy_matrix(C_temp, C_loc+(i*n_loc_min)+res, n_loc, n_loc_min, n);
    end_cpy_time = MPI_Wtime();
  }
#else
...
#ifdef CUDA
  cpy_time += end_cpy_time - start_cpy_time;
#endif
 }
end_time = MPI_Wtime();
 ...
 #ifdef __CUDA
  cuda_stop();
  calc_time = end_time-start_time-(cpy_time-t_cpy_Aloc)-comm_time;
 #endif

 tot_time = end_time-start_time+t_cpy_Aloc;
...
#ifdef __CUDA
 MPI_Reduce(&init_time, &all_init_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
 MPI_Reduce(&cpy_time, &all_cpy_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
 ...
 if(rank == 0) {
  printf("\nTimes\n");
#ifdef __CUDA
  printf("Time for Initializing the GPU is %f\n", all_init_time/np);
