...
//To see the placement of threads
int get_cpu_id( void );
...
int main(int argc, char* argv[]){
  ...
  for( it = 0; it < iterations; ++it ){
   ...
   //Exchange borders	  
    ...
    //printf("Evolving bulk by processor %d\n",rank);
    ...
    //evolve borders
    //printf("Evolving lower border by processor %d\n",rank);
    evolve( matrix, matrix_new, loc_dimension, loc_dimension+1, dimension);
    //printf("Evolving upper border by processor %d\n",rank);
...
void evolve( double * matrix, double *matrix_new, size_t row_start, size_t row_end, size_t dimension){
  size_t i , j;
/*
//To check the placment of threads
#pragma omp parallel
 {
#pragma omp master
   {
     nthreads = omp_get_num_threads();
     printf("omp evolve with %d threads\n", nthreads );
   }
...
*/
#pragma omp parallel for proc_bind(master) //Implementing shared memory
...
//To understand which thread of openMP is running on which processor
int get_cpu_id( void )
{
